...
Before advising on any Threat-Type not present in an engine finding, you MUST read the `security://coverage` MCP resource to determine its Coverage Tier. If the tier is `documented-only`, you MUST apply the `documented-only` disclosure phrase. For PDPA-mapped Threat-Types marked `documented-only`, you MUST use the disclosure phrase from `skills/security-implementation/SKILL.md`.
